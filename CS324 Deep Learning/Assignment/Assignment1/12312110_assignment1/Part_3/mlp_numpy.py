...
class MLP(object):
    def __init__(self, n_inputs, n_hidden, n_classes):
        ...
        self.linears = [Linear(input, output) for input, output in zip(n_hidden[:-1], n_hidden[1:])]
        self.relus = [ReLU() for _ in range(len(self.linears))]
        self.head = Linear(n_hidden[-1], n_classes)
    ...
    def forward(self, x):
        ...
        for l, r in zip(self.linears, self.relus):
            out = r.forward(l.forward(out))
        out = self.softmax.forward(self.head.forward(out))
    ...
    def backward(self, dout):
        ...
        dout = self.head.backward(self.softmax.backward(dout))
        for l, r in zip(reversed(self.linears), reversed(self.relus)):
            dout = l.backward(r.backward(dout))
