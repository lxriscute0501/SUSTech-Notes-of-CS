...
found:
    ...
    p->remaining_quantum = FULL_QUANTUM;
    p->sched_count = 0;
    p->create_time = ticks;
    p->first_run_time = 0;
    p->run_ticks = 0;
...
void exit(int code) {
    ...
    int turnaround = (int)(finish_time - p->create_time);
    int running = (int)p->run_ticks;
    int waiting = turnaround > running ? turnaround - running : 0;
    printf("[sched] pid=%d priority=%d scheduled=%d running=%d waiting=%d turnaround=%d\n", p->pid, p->priority, p->sched_count, running, waiting, turnaround);
