...
void kernel_trap(struct ktrapframe *ktf) {
    ...
    } else if ((exception_code == LoadPageFault || exception_code == StorePageFault) && (r_sstatus() & SSTATUS_SUM) && curr_proc() != NULL && r_stval() <= MAX_USERVA) {
	struct proc *p = mycpu()->proc;
	w_sstatus(r_sstatus() & ~SSTATUS_SUM);
	assert(mycpu()->inkernel_trap == 1);
	mycpu()->inkernel_trap--;
	if (holding(&p->mm->lock)) release(&p->mm->lock);
	exit(-9);
	panic_never_reach();
}  else {
