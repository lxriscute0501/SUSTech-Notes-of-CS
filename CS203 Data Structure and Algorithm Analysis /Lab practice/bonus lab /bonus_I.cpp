...
int main()
{
    ...
    for (int i=1; i<=t; i++)
    {
        int n, x, y, k1=0, k2=0, ans=0, total=0;
        cin >> n;
        for (int j=1; j<=n; j++)
        {
            cin>>x>>y;
        ...
        if (k1 >= 1 and k2 >= 1) ans += max(min(red1[k1], blue2[1]), min(red2[1], blue1[k1])) * 2;
        cout<<total-ans<<endl;
    }
